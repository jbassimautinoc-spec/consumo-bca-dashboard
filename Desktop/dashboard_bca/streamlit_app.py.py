...
correo_ingresado = st.sidebar.text_input("Ingrese su correo empresarial para acceder al tablero:")

if not correo_ingresado:
    st.warning("⚠ Por favor, ingrese su correo en la barra lateral para continuar.")
    st.stop()

correo_normalizado = correo_ingresado.strip().lower()

if correo_normalizado not in USUARIOS_PERMITIDOS:
    st.error("🚫 El correo ingresado **no está autorizado** para acceder al tablero.\n\n"
             "Si considera que debería tener acceso, comuníquese con el administrador.")
    st.stop()

st.success(f"🔓 Acceso concedido: {correo_normalizado}")
